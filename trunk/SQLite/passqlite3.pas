...
uses {$IFNDEF LINUX}{$IFDEF FPC}LCLIntf, {$ENDIF}Windows, {$ENDIF}
        ...
        begin
          ...
          for i := 0 to Columns - 1 do
            begin
              ...
              inc (FCurrentSet.FQuerySize, length(String(Value^)));
              inc(Value);
            end;
          if FCurrentSet.FFields.Count = 0 then //do only once per query
            for i := 0 to Columns - 1 do
              begin
                j:=FCurrentSet.FFields.AddObject(FieldName^, TFieldDesc.Create);
...
//                    table := '';
      ...
      QueryCallBack := Integer ((not FCallBackOnly) and (FCurrentSet.FQuerySize>FFetchMemoryLimit)); //return 0 if ok for next row
    end;
end;
...
begin
  Result := False;
  FCurrentSet.FHasResult := False;
  if not LibsLoaded or
     (FBaseInfo = nil) then
    begin
      FCurrentSet.FLastErrorText := 'Failed to load dll library';
  ...
  FCurrentSet.FSQL:=SQL;
  if Assigned (OnBeforeQuery) then
    OnBeforeQuery(Self, FCurrentSet.FSQL);
  SQL:=FCurrentSet.FSQL;
  ...
  FCurrentSet.FQuerySize := 0;
  if SQL<>'' then  //calling with empty string causes a result cleanup.
    begin
      FCurrentSet.FLastError := SQLite3_Exec(FBaseInfo.FHandle, PChar(SQL), @QueryCallBack, Self, P);
      if FCurrentSet.FLastError = SQLITE_ERROR then begin
        SetString(FCurrentSet.FLastErrorText, P, StrLen(P));
        SQLite3_Free(P);
      end;
    end;
  i:=FCurrentSet.FRowList.Count - 1;
  if not FCallBackOnly then //we need to clean up
    begin
      while i>=FCurrentSet.FRowCount do
        begin
          TResultRow(FCurrentSet.FRowList[i]).Free;
          FCurrentSet.FRowList.Delete(i);
          dec(i);
        end;
    end;
  FCurrentSet.FRowsAffected := SQLite3_Changes(FBaseInfo.FHandle);
  //we need to do this here (multi-threaded!):
  FCurrentSet.FLastInsertID := SQLite3_LastInsertRowId(FBaseInfo.FHandle);
  FBaseInfo.CS.Leave;
  FCurrentSet.FColCount := FCurrentSet.FFields.Count;
  Result := (FCurrentSet.FLastError = sqlite_ok);
  FCurrentSet.FHasResult := Result;
...
begin
  ...
  if not LibsLoaded then begin
    FCurrentSet.FLastErrorText := 'ups failed to load libs';
    Exit;
  end;
